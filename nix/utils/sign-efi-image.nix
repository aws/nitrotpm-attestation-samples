...
let
  ...
  script = pkgs.writeShellApplication {
    ...
    runtimeInputs = (with pkgs; [
      ...
      awscli2
      nitrotpm-tools
    ]) ++ uefivars.runtimeInputs;

    text = ''
      set -euo pipefail

      usage() {
        echo "Usage: sign-efi-image <image-dir> <keys-dir> [--identity-arn <ARN>] [> tpm_pcr.json]" >&2
        echo "" >&2
        ...
        echo "             (and db.key unless --identity-arn is used)" >&2
        echo "" >&2
        echo "Options:" >&2
        echo "  --identity-arn <ARN>  Fetch the golden identity JSON from AWS" >&2
        echo "             Secrets Manager and stream its db signing key (PEM)" >&2
        echo "             straight into sbsign via an in-memory fd. The private" >&2
        echo "             key is never written to disk; no db.key file is read" >&2
        echo "             from keys-dir. Without this flag, keys-dir/db.key is" >&2
      ...
      # Print the identity secret's db signing key (PEM) to stdout for a process
      # substitution (see the signing block). The key lives only in a local var
      # and a pipe, never a file. Exits non-zero on a missing secret or empty
      # db_key so an unreadable ARN can't silently feed empty input downstream.
      fetch_db_key() {
        local arn="$1" json key
        if ! json=$(aws secretsmanager get-secret-value \
              --secret-id "$arn" --query SecretString --output text); then
          echo "Error: failed to retrieve identity from Secrets Manager ($arn)" >&2
          return 1
        fi
        key=$(printf '%s' "$json" | jq -r '.db_key // empty')
        if [ -z "$key" ]; then
          echo "Error: identity secret has no db_key field ($arn)" >&2
          return 1
        fi
        printf '%s' "$key"
      }
      ...
      # Optional: fetch the golden identity from Secrets Manager and extract its
      # db signing key in memory, instead of reading a db.key file.
      IDENTITY_ARN=""
      while [ "$#" -gt 0 ]; do
        case "$1" in
          --identity-arn)
            if [ "$#" -lt 2 ]; then
              echo "Error: --identity-arn requires an ARN argument" >&2
              exit 1
            fi
            IDENTITY_ARN="$2"
            shift 2
      ...
      # db.key is only required as a file when not fetching the identity from
      # Secrets Manager. db.crt + the ESLs are public and always come from
      # keys-dir.
      ...
      if [ -n "$IDENTITY_ARN" ]; then
        # Stream the signing key from Secrets Manager into sbsign via a
        # process-substitution fd — never a file. sbsign reads it through
        # OpenSSL's PEM BIO, which accepts a non-seekable fd.
        echo "Fetching db signing key from Secrets Manager and signing UKI (no disk)..." >&2
        sbsign --key <(fetch_db_key "$IDENTITY_ARN") \
